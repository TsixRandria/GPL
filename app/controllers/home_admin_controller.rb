class HomeAdminController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @voiture.save
        format.html { redirect_to homepage_admin_path, notice: 'Voiture was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @voiture.update(voiture_params)
        format.html { redirect_to homepage_admin_path, notice: 'Voiture was successfully updated.' }
  ...
  private
    ...
    def voiture_params
      params.require(:voiture).permit(:image, :marque, :place, :vitesse, :types, :climatiseur, :status)
    end
end
